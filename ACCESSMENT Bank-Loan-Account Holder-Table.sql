...
Balance int,
Account_Holders_no int primary key,
Account_Holders_id int ,
foreign key (Account_holders_id) references loan6 (Account_holders_id)
);
...
insert into account_holder8 values
...
select * from account_holder8;

select * from account_holder8 where City = "Palanpur"; /*Also fetch the details of the account holder who are related from the same city */

select Account_Holders_no,Account_holder_name from account_holder8 where Date_of_account_created >= 15;/*Write a query to fetch account number and account holder name, whose 
accounts were created after 15th of any month*/

select City,count(Branch_city),Branch_city as Count_branch from account_holder8,bank3 group by Branch_City,City;/*Write a query to display the city name and count the branches in that city. 
Give the count of branches an alias name of Count_Branch.*/

select Bramch_id,Loan_amount from bank3 left join loan7 on bank3.Bramch_id = loan7.Branch_id;

select Account_Holders_id from account_holder8 right join loan7 on account_holder8.Account_holders_id = loan7.Account_Holders_id ;